...
const Projects = () => {
  ...
  /* ===== TEXT PROGRAM ELEGANCE ===== */
  const projectTitles = [
    "Pelatihan Attitude & Karakter",
    "Public Speaking & Komunikasi",
    "Personal Branding Program",
    "Workshop & Konsultasi Profesional",
  ];

  return (
    <Container ref={containerEl}>
      <Top ref={topEl}>
        <ParagraphWithLightBorder>Program Unggulan</ParagraphWithLightBorder>
        <TitleWithBigMargin>
          Program Pengembangan Attitude & Komunikasi Berkelas
        </TitleWithBigMargin>
      </Top>
      ...
      <CardContainer className="project-slick-list" ref={slideEl} {...settings}>
        {projectTitles.map((title, idx) => (
          <Card key={idx}>
            <CardImage
              onLoad={handleLoad}
              src={`${imgbaseUrl}project-${idx + 1}.png`}
            />
            <CardLinkContainer>
              <CardLink to="/programs" />
